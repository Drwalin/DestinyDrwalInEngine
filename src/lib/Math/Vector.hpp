...
public:
    ...
    inline Vector operator -  ( Vector src )const;   // odjêcie wektora
    inline Vector operator -= ( Vector src );        // odjêcie wektora
    ...
    inline bool   operator == ( Vector src )const;   // sprawdzenie równoœci wektorów
    inline bool   operator != ( Vector src )const;   // sprawdzenie nierównoœci wektorów
    inline bool   operator <  ( const float  src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest mniejsza od src
    inline bool   operator >  ( const float  src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest wiêksza od src
    inline bool   operator <  ( Vector src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest mniejsza od src
    inline bool   operator >  ( Vector src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest wiêksza od src
    inline bool   operator <= ( Vector src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest mniejsza od src
    inline bool   operator >= ( Vector src )const;   // sprawdzenie ka¿dej sk³adowej wektor czy jest wiêksza od src
...
inline Vector Vector::operator +( Vector src )const   // zwraca sumê wektorów
{
    Vector out;
...
inline Vector Vector::operator -( Vector src )const   // zwraca ró¿nicê wektorów
{
    Vector out;
    out.vector[0] = vector[0] - src.vector[0];
    out.vector[1] = vector[1] - src.vector[1];
...
inline bool   Vector::operator == ( Vector src )const   // sprawdzenie równoœci wektorów
{
...
inline bool   Vector::operator != ( Vector src )const   // sprawdzenie nierównoœci wektorów
{
    if( (!memcmp( vector, src.vector, 3*sizeof(src.vector[0]) )) != 1 )
        return true;
    else
...
inline bool   Vector::operator >  ( Vector src )const   // sprawdzenie ka¿dej sk³adowej wektor czy jest wiêksza od src
{
	if( vector[0] > src[0] )
		if( vector[1] > src[1] )
			if( vector[2] > src[2] )
